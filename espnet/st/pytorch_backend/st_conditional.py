...
def trans(args):
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                nbest_asr_hyps, nbest_mt_hyps = model.translate(
                    feat,
                    js[name]["output"][1]["tokenid"],
                    args,
                    train_args.char_list,
                    rnnlm,
                )
                new_asr_js[name] = add_results_to_json(
                    js[name], nbest_asr_hyps, train_args.char_list, idx=1
                )
                new_mt_js[name] = add_results_to_json(
                    js[name], nbest_mt_hyps, train_args.char_list
                )

    else:
        raise NotImplementedError("only support batch size = 0")

    asr_result_label = os.path.splitext(args.result_label)[0] + ".asr"
    with open(asr_result_label, "wb") as f:
        f.write(
            json.dumps(
                {"utts": new_asr_js}, indent=4, ensure_ascii=False, sort_keys=True
            ).encode("utf_8")
        )
    ...
    with open(args.result_label, "wb") as f:
        f.write(
            json.dumps(
                {"utts": new_mt_js}, indent=4, ensure_ascii=False, sort_keys=True
            ).encode("utf_8")
        )

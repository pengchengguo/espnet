...
class E2E(STInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_pad_src):
        ...
        # 2. forward lstm
        n_batch, tmax, dunits = hs_pad.size()
        hs_pad = hs_pad.view(n_batch * tmax, dunits)
        h_init = hs_pad.new_zeros(hs_pad.size(0), self.adim)
        c_init = hs_pad.new_zeros(hs_pad.size(0), self.adim)
        lstm_h, lstm_c = self.lstm(hs_pad, (h_init, c_init))

        hs_pad = lstm_h.view(n_batch, tmax, dunits)

        # 3. compute asr loss
        loss_asr_att, acc_asr, cer, wer, ys_pad_src_asr = self.forward_asr(
        ...
        if random.random() < self.sampling_probability:
            ...
            ys_pad_input = ys_pad_src
        else:
            ys_pad_input = ys_pad_src_asr

        hs_pad_mt, hs_pad_mt_mask = self.forward_mt(ys_pad_input, ys_pad_src)
        hs_pad_mt_attn = self.attn_mt(hs_pad, hs_pad_mt, hs_pad_mt, hs_pad_mt_mask)

        # 5. forward lstm
        n_batch, tmax, dunits = hs_pad_mt_attn.size()
        hs_pad_mt_attn = hs_pad_mt_attn.view(n_batch * tmax, dunits)
        lstm_h, lstm_c = self.lstm(hs_pad_mt_attn, (lstm_h, lstm_c))

        hs_pad_mt_attn = lstm_h.view(n_batch, tmax, dunits)
        ...
        pred_pad, pred_mask = self.decoder(ys_in_pad, ys_mask, hs_pad_mt_attn, hs_mask)
        ...
        self.loss = loss_asr_att + loss_st

        loss_asr_data = float(loss_asr_att)
    ...
    def forward_asr(self, hs_pad, hs_mask, ys_pad):
        ...
        return loss_att, acc, cer, wer, ys_hat_asr[:, :-1]

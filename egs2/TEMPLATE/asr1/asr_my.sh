...
tgtspk_asr=false        # Whether to perform target speaker ASR task.
enroll_prefix=xvector   # Prefix of enrollment scp file.
enroll_type=kaldi_ark   # Type of target speaker enrollment, e.g., kaldi_ark, sound
...
Options:
    ...
    --tgtspk_asr        # Whether to perform target speaker ASR task (default="${tgtspk_asr}")
    --enroll_prefix     # Prefix of enrollment scp file (default="${enroll_prefix}")
    --enroll_type       # Type of target speaker enrollment (default="${enroll_type}")
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if ${tgtspk_asr}; then
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/${enroll_prefix}.scp,enroll,${enroll_type} "
        # _opts+="--valid_shape_file ${asr_stats_dir}/valid/${enroll_prefix}_shape "
    fi
    if ${use_prompt}; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/prompt,prompt,text "
    ...
    for dset in ${_dsets}; do
        ...
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            CUDA_VISIBLE_DEVICES=JOB ${python} -m espnet2.bin.${asr_task}_inference${inference_bin_tag} \
                ...
                ${_opts} ${_other_inputs} ${inference_args} || { cat $(grep -l -i error "${_logdir}"/asr_inference.*.log) ; exit 1; }

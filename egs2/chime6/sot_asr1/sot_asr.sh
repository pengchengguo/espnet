...
speaker_inventory=false    # Speaker inventory file for ASR input
speaker_inventory_size=1   # Number of speaker embeddings in the speaker inventory
text_suf=   # text suffix to use different text files.
...
Options:
    ...
    --speaker_inventory    # Whether to use speaker inventory for ASR input (default="${speaker_inventory}")
    --speaker_inventory_size   # Number of speaker embeddings in speaker inventory (default="${speaker_inventory_size}")
    --text_suf=  # text suffix to use different text files (default="${text_suf}").
...
if [ ${num_ref} -eq 1 ]; then
    ...
    ref_text_files_str="text${text_suf:+.$text_suf} "
    ...
    ref_text_files_str="text_spk1${text_suf:+.$text_suf} "
    ref_text_names_str="text "
    for n in $(seq 2 ${num_ref}); do
        ref_text_files_str+="text_spk${n}${text_suf:+.$text_suf} "
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        if [ -n "${speed_perturb_factors}" ]; then
            ...
            extra_files=
            if [ -n "${ref_text_files_str}" ]; then
                extra_files+="${ref_text_files_str} "
            fi
            if ${speaker_inventory}; then
                extra_files+="speakers.scp "
            fi
            for factor in ${speed_perturb_factors}; do
               if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                   scripts/utils/perturb_data_dir_speed.sh \
                       ${extra_files:+--utt_extra_files "${extra_files}"} \
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                if ${speaker_inventory}; then
                    for speaker_file in speakers.scp; do
                        [ -f data/${dset}/${speaker_file} ] && cp data/${dset}/${speaker_file} ${data_feats}${_suf}/${dset}
                    done
                fi
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            if ${speaker_inventory}; then
                for speaker_file in speakers.scp; do
                    extra_files+=" ${speaker_file}"
                    <"${data_feats}/org/${dset}/${speaker_file}" \
                        awk ' { if( NF != 1 ) print $0; } ' >"${data_feats}/${dset}/${speaker_file}"
                done
            fi
        ...
        done > "${data_feats}/lm_train${text_suf:+_$text_suf}.txt"
    fi
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        if [ "${token_type}" = bpe ]; then
            ...
            if ${sot_asr}; then
                if [ ${speaker_inventory_size} -eq 1 ]; then
                    _opts_spm+=" --user_defined_symbols=<sc>"
                elif [ ${speaker_inventory_size} -gt 1 ]; then
                    _opts_spm+=" --user_defined_symbols=<sc1>"
                    for i in $(seq 2 ${speaker_inventory_size}); do
                        _opts_spm+=",<sc${i}>"
                    done
                fi
            fi
            ...
            if ${sot_asr} && [ "${token_type}" = char ]; then
                if [ ${speaker_inventory_size} -eq 1 ]; then
                    ...
                    for i in $(seq 1 ${speaker_inventory_size}); do
                        _opts+=" --add_nonsplit_symbol <sc${i}>:$(( i+1 )) "
                    done
                fi
            fi
            ...
            ${python} -m espnet2.bin.tokenize_text  \
                ...
                --input "${data_feats}/lm_train${text_suf:+_$text_suf}.txt" --output "${token_list}" ${_opts} \
                --field 2- \
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
        ...
        if ${use_word_lm} && [ "${token_type}" != word ]; then
            log "Generate word level token_list from ${data_feats}/lm_train${text_suf:+_$text_suf}.txt"
            ${python} -m espnet2.bin.tokenize_text \
                --token_type word \
                --input "${data_feats}/lm_train${text_suf:+_$text_suf}.txt" --output "${lm_token_list}" \
                --field 2- \
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --write_vocabulary true \
...
if ! "${skip_train}"; then
    if "${use_lm}"; then
        if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
            log "Stage 6: LM collect stats: train_set=${data_feats}/lm_train${text_suf:+_$text_suf}.txt, dev_set=${lm_dev_text}"
            ...
            key_file="${data_feats}/lm_train${text_suf:+_$text_suf}.txt"
            split_scps=""
            for n in $(seq ${_nj}); do
            ...
            ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
                ${python} -m espnet2.bin.lm_train \
                    ...
                    --cleaner "${cleaner}" \
                    --g2p "${g2p}" \
                    --train_data_path_and_name_and_type "${data_feats}/lm_train${text_suf:+_$text_suf}.txt,text,text" \
        ...
        if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
            log "Stage 7: LM Training: train_set=${data_feats}/lm_train${text_suf:+_$text_suf}.txt, dev_set=${lm_dev_text}"
            ...
            if [ "${num_splits_lm}" -gt 1 ]; then
                ...
                _opts+="--train_data_path_and_name_and_type ${_split_dir}/lm_train${text_suf:+_$text_suf}.txt,text,text "
                _opts+="--train_shape_file ${_split_dir}/text_shape.${lm_token_type} "
                _opts+="--multiple_iterator true "

            else
                _opts+="--train_data_path_and_name_and_type ${data_feats}/lm_train${text_suf:+_$text_suf}.txt,text,text "
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if ${sot_asr} && ${speaker_inventory}; then
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/speakers.scp,speaker_inventory,kaldi_ark "
            _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/speakers.scp,speaker_inventory,kaldi_ark "
        fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            for i in ${!ref_text_names[@]}; do
                if [ ${i} -gt 1 ]; then
                    extra_split_scps+="${_asr_train_dir} ${ref_text_files[$i]} "
                    extra_split_scps+="${asr_stats_dir}/train/${ref_text_names[$i]}_shape.${token_type} "
                fi
            done
            if ${sot_asr} && ${speaker_inventory}; then
                extra_split_scps+="${_asr_train_dir} speakers.scp "
                extra_split_scps+="${asr_stats_dir}/train/speaker_inventory_shape "
            fi
            if [ ! -f "${_split_dir}/.done" ]; then
                rm -f "${_split_dir}/.done"
                ${python} -m espnet2.bin.split_scps \
            ...
            if ${sot_asr} && ${speaker_inventory}; then
                _opts+="--fold_length ${asr_speech_fold_length} "
                _opts+="--train_data_path_and_name_and_type ${_split_dir}/speakers.scp,speaker_inventory,kaldi_ark "
                _opts+="--train_shape_file ${_split_dir}/train/speaker_inventory_shape "
            fi
            _opts+="--multiple_iterator true "
            ...
            if ${sot_asr} && ${speaker_inventory}; then
                _opts+="--fold_length ${asr_speech_fold_length} "
                _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/speakers.scp,speaker_inventory,kaldi_ark "
                _opts+="--train_shape_file ${asr_stats_dir}/train/speaker_inventory_shape "
            fi
        fi
        ...
        if ${sot_asr} && ${speaker_inventory}; then
            _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/speakers.scp,speaker_inventory,kaldi_ark "
            _opts+="--valid_shape_file ${asr_stats_dir}/valid/speaker_inventory_shape "
        fi
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if ${sot_asr} && ${speaker_inventory}; then
                _opts+="--data_path_and_name_and_type ${_data}/speakers.scp,speaker_inventory,kaldi_ark "
            fi
            ...
            # shellcheck disable=SC2068
            for ref_txt in ${ref_text_files[@]}; do
                # suffix=$(echo ${ref_txt} | sed 's/text//; s/\.${text_suf}//')
                suffix=
        ...
        for dset in ${test_sets}; do
            ...
            for _tok_type in "char" "word" "bpe"; do
                ...
                for ref_txt in ${ref_text_files[@]}; do
                    ...
                    suffix=
                    hyp_txt=${ref_txt}
                    [ -n "${text_suf}" ] && hyp_txt=$(echo ${ref_txt} | sed -e "s/\.${text_suf}//")

...
for outidx in $(seq ${num_spkrs}); do
  ...
  if [ -f ${dir}/ctc_alignment${suffix} ]; then
    cat ${dir}/ctc_alignment${suffix} > ${tmpdir}/output/ctc_alignment${suffix}.scp
    < ${tmpdir}/output/ctc_alignment${suffix}.scp utils/sym2int.pl --map-oov ${oov} -f 2- ${dic} > ${tmpdir}/output/ctc_alignmentid${suffix}.scp
  fi
  ...
  if [ -f ${dir}/ctc_alignment${suffix} ]; then
    output_strs=${output_strs}"ctc_alignment:${tmpdir}/output/ctc_alignment${suffix}.scp \
                               ctc_alignmentid:${tmpdir}/output/ctc_alignmentid${suffix}.scp "
  fi
done
